...
​		突然冒出来的思路：我们把每个儿子节点的儿子，每个儿子到脖子节点看做一个手，总数记为$x$，那么手的总数为$C^2_x$，因为可能会选重复，所以我们记录手选在同一个子树的情况，让$sson=\sum C_{son[j]}^2$，$j$为脖子的儿子，$son[j]$为$j$的儿子数量,此时我们枚举躯干$j$,手的情况数就是$C_{x-(son[j]-1)}^2-sson+C_{son[j-1]}^2$，脚的情况为$C_{son[j-1]}^2$，头的情况数为$son[i]-1$($i$为脖子节点),乘上就可以了
